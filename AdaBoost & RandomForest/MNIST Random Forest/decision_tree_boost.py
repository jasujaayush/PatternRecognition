...
class Tree:	
	...
	def giniIndexOld(self,groups,classes):
		gini = 0.0
		total = 0
		for group in groups:
			total += sum(group[1])
		...
		for group in groups:
			...
			if size == 0:
				continue
			temp = np.array([row[0] for row in group[0]])
			for class_value in classes:	
				indices = np.where(temp == class_value)
				g = np.array(group[1])
				proportion = sum(g[indices]) / float(size)
	...
	def getSplit(self,current_set, data_weight):
		...
		for index in range(1, M):
			for row in current_set:
				groups = self.randomSplit(index, row[index], current_set, weights)
				gini = self.giniIndex(groups, classes)
				if gini > b_score:
					b_index, b_value, b_score, b_groups = index, row[index], gini, (groups[0][0],groups[1][0])
